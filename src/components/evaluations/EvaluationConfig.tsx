...
export default function EvaluationConfig({ params }: EvaluationConfigProps) {
  ...
  // Fetch audio files when audio tab is active
  useEffect(() => {
    if (activeTab === 'audio') {
      fetchAudioFiles()
    }
  ...
  const fetchAudioFiles = async () => {
    ...
    try {
      const response = await fetch(`/api/audio-upload?project_id=${params.projectid}&agent_id=${params.agentid}`)
              ...
              <CardContent>
                {loadingAudioFiles ? (
                  ...
                  <div className="text-center py-12">
                    ...
                    <p className="text-gray-600">Upload audio files to start processing them for evaluation.</p>
                  </div>
                ) : (
                  <div className="space-y-2">
                    {audioFiles.map((file) => (
